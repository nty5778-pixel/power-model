...
v17 실측에서 정상 동작 확인:
```
RT 구간행 220 → 시간 56 (구간수분포 {1:1, 3:1, 4:54}) → 완결(n>=4) 54
DA∩RT 교집합 54시간 (2026-08-04 12:00 ~ 2026-08-06 17:00) → 신규 54행
```
다만 상한(220)에 걸려 **한 번에 2.3일치만** 회수됐다. MIS는 RTM을 8발행일(746문서) 보관하므로
8일 전부를 받으려면 768문서가 필요하다.

**v18 수집 순서**: 최신 발행일 우선(운영 최신성) → 나머지는 **오래된 순**(백필 수렴).
상한을 넘겨도 매 실행 진도가 나가고, `_covered()`가 이미 받은 시간대를 건너뛰므로 중복 수신이 없다.

시뮬레이션(8일=192시간, 상한 260):
```
1회차 신규 65행 · 누적  65   잔여 508
2회차 신규 63행 · 누적 128   잔여 256
3회차 신규 63행 · 누적 191   잔여   4
4회차 신규  1행 · 누적 192   완료
```
**수동 실행 3~4회로 8일치가 채워진다.** 이후 일일 실행은 하루치(~96문서)만 받는다.

기본값: `rtm_days=7`, `max_rtm_docs=260`. `/score` 요청 body로도 조절 가능
(`{"rtm_days":7,"max_rtm_docs":400}`). `/settle?max_rtm_docs=400` 로 미리 확인 가능.

### 영구 한계
- **DAM(12331)은 25일 보관** — DA는 2026-07-13까지 복원됨
- **RTM(12301)은 8발행일 보관** — RT는 약 8일치만 복원 가능
- 따라서 **2026-07-25~07-29 판정은 영구히 채점 불가**. 그 이후는 매일 실행하면 누적된다.

### 실행 시간
문서 260개 수신에 2~4분. n8n `POST /score` 타임아웃 600초 안에 들어오지만,
Render 콜드스타트까지 겹치면 첫 실행이 특히 느리다.
